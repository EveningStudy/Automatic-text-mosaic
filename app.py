...
def upload_image():
    if request.method == 'POST':
        ...
        if file:
            ...
            if action == 'automatic':
                return redirect(url_for('automatic_mosaic', filename=filename, mosaic_type=mosaic_type, color=color_hex))
            elif action == 'manual':
...
def automatic_mosaic(filename):
    ...
    file_path = os.path.join(app.config['UPLOAD_FOLDER'], filename)
    detected_boxes = np.load(os.path.join(app.config['PROCESSED_FOLDER'], 'boxes_' + filename + '.npy'), allow_pickle=True)
